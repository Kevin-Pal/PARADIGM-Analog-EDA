...
class TurboM_Plus(Turbo1):
    """TuRBO-m with parallel evaluation, warm starts, iteration limits, and early stopping.

    Parameters
    ----------
    f : Callable accepting a 2D candidate array and returning a 2D objective array.
    lb, ub : Lower and upper bounds with shape (d,).
    n_init : Initial points per trust region; used after warm-start reinitialization.
    max_evals : Total evaluation budget.
    n_trust_regions : Number of trust regions.
    batch_size : Candidates evaluated per trust region and round.
    ...
    x_init, fx_init : Optional warm-start points and corresponding values.
    ...
    def optimize(self):
        ...
        if self.x_init is not None:    
            # Randomly partition the supplied points and values across n_trust_regions.
            if self.verbose:
        ...
        # Use iter_num to track the iteration count.
        iter_num = 0
